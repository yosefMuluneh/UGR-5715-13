...
function signup(){
    ...
    let theSignUp = document.createElement("div")
        theSignUp.classList = "recievers  pb-5 fixed-top"
        theSignUp.innerHTML = `
            <div class="recieve-block w-50 rounded ms-5 me-5 pt-5 pb-5 bg-info ps-5 form-container">
                <form>
                <div class="form-container form-row">
                    ...
                    <input id="conf-password" class="form-control" type="password" />
                    </div>
                    </div>
                </form>
                <div class="but-cont">
                <button class="only-create btn btn-primary" onclick="onlyCreate()">Create user</button>
                <button class="create-login btn btn-success" onclick="createLogin()">Create and login</button>
                </div>
            </div>
        `
...
function setSrc(){
    ...
    if(theLiked.length > 0){
        ...
        for (let itBe = 0; itBe<theLiked.length; itBe++){
            ...
            theImages[itBe+2].src = (theLiked[trackList] !== null ? theLiked[trackList]:"./images/nature3.jpg")
    ...
    remvBtn.forEach(btn=>{
        ...
        btn.addEventListener('click',()=>{
            ...
            if(btn.parentElement.previousElementSibling){
                ...
                btn.parentElement.classList.add("third-btn-cont")
            }else{
                theImgSrc = btn.parentElement.firstElementChild.getAttribute("src")
                //btn.parentElement.style.opacity = 0
            }
